...
destination_loc = sys.argv[1]
dataset_loc = sys.argv[2]

if not (destination_loc or dataset_loc):
    print("Usage: python finetuning.py <destination_location> <dataset_location>")
    sys.exit(1)
...
label_list = [
    ...
    "B-geo",  # Beginning of a miscellaneous entity right after another miscellaneous entity
    "I-geo",  # Miscellaneous entity
    "B-org",   # Beginning of a person's name right after another person's name
    "I-org",   # Person's name
    "B-per",   # Beginning of an organisation right after another organisation
    "I-per",   # Organisation
    "B-gpe",   # Beginning of a location right after another location
    "I-gpe",    # Location
    "B-tim",
    "I-tim",
    "B-art",
        ...
        for line in lines: # This is one of the entries in the dataset
            ...
            for idx, t in enumerate(json_obj['tokens']):
                ...
                if 0 == len(ids):
                    ...
                    labels.extend(get_labels(label_list.index(json_obj["labels"][idx]), len(ids)))
...
ds_val, ds_train = random_split(dataset, [500, len(dataset)-500])
loader_train = DataLoader(ds_train, batch_size=BATCH_SIZE)
loader_val = DataLoader(ds_val, batch_size=BATCH_SIZE)
model_config = destination_loc
model = BertForTokenClassification.from_pretrained(pretrained_model_name_or_path="bert-large-cased", config=model_config)
model.to('cuda')
model.train()
...
def evaluate(net, loss, dataloader):
    ...
    with torch.no_grad():
        for tokens, labels in dataloader:
            tokens, labels = tokens.cuda(), to_one_hot(labels, NUM_LABELS).cuda()
...
def train(model, opti, loss, loader):
    for epoch in range(EPOCHS):
        for i, (tokens, labels) in enumerate(loader):
            ...
            #[tokens, labels] = i
            tokens, labels = tokens.cuda(), to_one_hot(labels, n_dims=NUM_LABELS).cuda()
            out = model(tokens).logits
            for idx, j in enumerate(tokens):
                token_len = np.where(j.cpu().numpy() == 102)[0][0]
